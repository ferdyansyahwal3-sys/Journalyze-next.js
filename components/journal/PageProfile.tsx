// components/journal/PageProfile.tsx — v2 fix: plan kolom + avatar debug
'use client';
...
export default function PageProfile({ active, onOpenApiKey, onOpenNotif }: ProfilePageProps) {
  ...
  useEffect(() => {
    ...
    // Ambil semua kolom relevan dari profiles
    _sb.from('profiles')
      .select('avatar_url, is_activated, activated_at, license_key, plan, plan_activated_at, plan_expires_at')
      .eq('id', currentUser.id)
      .maybeSingle()
      .then(({ data, error }) => {
        if (error) console.error('[PageProfile] profiles fetch error:', error.message);
        if (!data) return;

        if (data.avatar_url) setAvatarUrl(data.avatar_url);

        // Tentukan plan label:
        // Prioritas: kolom `plan` (text) → fallback ke `is_activated`
        let planLabel = 'Free';
        if (data.plan) {
          // Normalize: 'pro', 'Pro', 'PRO' → 'Pro'
          const p = String(data.plan).toLowerCase();
          if (p === 'pro')   planLabel = 'Pro';
          else if (p === 'elite') planLabel = 'Elite';
          else if (p === 'basic') planLabel = 'Basic';
          else planLabel = data.plan; // keep as-is kalau ada nilai lain
  ...
  // ── Upload avatar — dengan fallback path & debug ──
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (dbErr) {
        ...
        showToast('❌ Gagal simpan URL foto', 'error');
        setAvatarLoading(false);
        return;
      }
  ...
  // Helper: tanggal aktif (prioritas plan_activated_at → activated_at)
  const activeDate = subscription?.planActivatedAt || subscription?.activatedAt;
    ...
    <div className="page active" id="page-profile">
      ...
      <div className="box ai-anim d1 pp-hero-card">
        <div className="pp-hero-top">
          ...
          <div className="pp-avatar-wrap">
            <div className="pp-avatar" onClick={() => fileRef.current?.click()} title="Klik untuk ganti foto">
              {avatarUrl
          ...
          <div className="pp-hero-info">
            ...
            <div className="pp-badges">
              {subscription && (
                <span className={`rbadge ${isPro ? 'mod' : 'agg'}`} style={{ fontSize: 8 }}>
                  {isPro ? '⭐' : '🔒'} Journalyze {subscription.plan}
                </span>
              )}
      ...
      <div className="g2 ai-anim d2" style={{ marginBottom: 16, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="box">
            <div className="box-head"><span className="box-title">👤 Nama Tampilan</span></div>
            <div className="box-body">
              <div className="fg" style={{ marginBottom: 0 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  />
                  <button className="btn btn-gold btn-sm" onClick={handleSaveName} disabled={nameLoading} style={{ flexShrink: 0 }}>
                    {nameLoading ? '⏳' : '✓ Simpan'}
                  </button>
                </div>
              </div>
          ...
          <div className="box">
            <div className="box-head"><span className="box-title">🔑 Ganti Password</span></div>
            <div className="box-body" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <div className="fg">
                <label className="flabel">Password Baru</label>
                <input className="finput" type="password" placeholder="Min. 6 karakter" value={passNew} onChange={(e) => setPassNew(e.target.value)} />
              </div>
              <div className="fg">
                <label className="flabel">Ulangi Password</label>
                <input className="finput" type="password" placeholder="Ulangi password baru" value={passNew2} onChange={(e) => setPassNew2(e.target.value)} />
              </div>
              <button className="btn btn-ghost btn-sm" onClick={handleChangePassword} disabled={passLoading} style={{ alignSelf: 'flex-end', marginTop: 4 }}>
      ...
      <div className="box ai-anim d3" style={{ marginBottom: 16 }}>
        <div className="box-head"><span className="box-title">⚙️ Preferensi Aplikasi</span></div>
        <div style={{ padding: 0 }}>
          ...
          <div className="pp-pref-row">
            <div>
              ...
              <div className="pp-pref-sub" style={{ color: apiKeyStatus.active ? 'var(--green)' : 'var(--text3)' }}>
                {apiKeyStatus.active ? `✅ Terhubung — ${apiKeyStatus.provider}` : 'Belum terhubung — analisis foto belum aktif'}
              </div>
            </div>
            <button
      ...
      {subscription && (
        <div className="box ai-anim d4" style={{ marginBottom: 16 }}>
          <div className="box-head">
            ...
            <span className={`chip ${isPro ? 'chip-gold' : 'chip-blue'}`}>{subscription.plan}</span>
          </div>
          <div className="box-body">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
              <div>
                <div style={{
                  ...
                  color: isPro ? 'var(--gold2)' : 'var(--text2)',
                  marginBottom: 6,
                }}>
                ...
                {activeDate && (
                  <div style={{ fontSize: 11, color: 'var(--text3)' }}>
                    Aktif sejak {new Date(activeDate).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                  </div>
                )}
                {subscription.planExpiresAt && (
                  <div style={{ fontSize: 11, color: 'var(--gold)', marginTop: 2 }}>
                    Berlaku hingga {new Date(subscription.planExpiresAt).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                  </div>
                )}
              </div>
              {!isPro && (
      ...
      <style>{`
        ...
        .pp-name { font-family: 'Cormorant Garamond', serif; font-size: 28px; font-weight: 700; line-height: 1.1; color: var(--text); margin-bottom: 5px; }
        .pp-email { font-family: 'JetBrains Mono', monospace; font-size: 11px; color: var(--text3); letter-spacing: .3px; margin-bottom: 12px; }
        .pp-badges { display: flex; gap: 7px; flex-wrap: wrap; align-items: center; }
        .pp-stats-strip { display: grid; grid-template-columns: repeat(4, 1fr); border-top: 1px solid var(--border2); }
        .pp-stat-item { padding: 14px 16px; text-align: center; }
        .pp-stat-lbl { font-family: 'JetBrains Mono', monospace; font-size: 7.5px; letter-spacing: 1.5px; text-transform: uppercase; color: var(--text4); margin-bottom: 5px; }
        .pp-stat-val { font-family: 'Cormorant Garamond', serif; font-size: 22px; font-weight: 700; line-height: 1; }
        .pp-pref-row { display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 14px 18px; }
        ...
        @media (max-width: 600px) {
          ...
          .pp-stat-item:nth-child(2) { border-right: none !important; }
          .pp-stat-item:nth-child(1), .pp-stat-item:nth-child(2) { border-bottom: 1px solid var(--border2); }
